...
export default function AdminCarousel() {
  ...
  const handleToggle = async (itemId) => {
    try {
      await api.patch(`/carousel/${itemId}/toggle`, {}, headers)
      fetchCarouselItems()
      toast.success('Carousel item status updated successfully.')
    } catch (error) {
      toast.error(error.response?.data?.message || error.message)
    }
  }
  ...
  const parseBulkCarouselRows = (rows) =>
    rows.map((row) => ({
      title: row.title || '',
      description: row.description || row.descriptionText || '',
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const parsedItems = parseBulkCarouselRows(rows)
      const errors = []

      parsedItems.forEach((item, index) => {
        if (!item.title) errors.push(`Row ${index + 2}: title is required.`)
        if (!item.image) errors.push(`Row ${index + 2}: image is required.`)
      })

      if (errors.length) {
        setBulkErrors(errors)
        setBulkPreviewRows(parsedItems.slice(0, 3))
        setBulkLoading(false)
        return
      }

      await api.post('/carousel/bulk', { items: parsedItems }, headers)
      toast.success('Bulk carousel items uploaded successfully.')
      setBulkFile(null)
      setBulkPreviewRows([])
      fetchCarouselItems()
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-700/30 bg-slate-900/70 p-5 shadow-lg">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-white">Bulk CSV Upload</h3>
            <p className="mt-1 text-sm text-slate-400">Upload carousel items from a CSV file. Use Excel and export as CSV.</p>
            <p className="mt-1 text-xs text-slate-500">
              📄 <a href="/sample-carousel.csv" target="_blank" className="text-indigo-400 hover:text-indigo-300 underline">Download sample CSV</a> for bulk upload format
            </p>
          </div>
        ...
        {bulkFile && (
          <div className="mt-4 rounded-2xl border border-slate-700/50 bg-slate-950/70 p-4 text-sm text-slate-300">
            <p className="font-semibold text-slate-100">CSV format:</p>
            <p className="mt-1">title,description,image,link,buttonText,order,isActive</p>
            <p className="mt-2 text-slate-400">Category is not needed here for carousel items.</p>
              ...
              <div className="mt-3 rounded-lg bg-slate-900/80 p-3 text-xs text-slate-300">
                ...
                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  {bulkPreviewRows.slice(0, 3).map((row, index) => (
                    <div key={index} className="rounded-lg bg-slate-950/70 p-3">
